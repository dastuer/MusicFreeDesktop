...
- 最近播放（主进程持久化，重启恢复）
- 用户歌单：创建 / 收藏单曲 / 侧边栏入口
- 音源插件管理：网络安装 / 本地安装、聚合订阅源批量导入与检查更新、启用禁用、排序、用户变量、卸载
...
侧边栏「音源」→ 输入插件 js 地址（如官方列表中的条目）或从本地 .js 文件安装。官方插件列表：https://github.com/maotoumao/MusicFreePlugins

也可以粘贴**聚合订阅源**链接（一份列出整批插件的 `index.json`）：一次装入它的全部音源，之后点「检查更新」按版本拉齐，删订阅源不影响已装音源。单个插件地址和聚合源地址填在同一个框里都能识别。
